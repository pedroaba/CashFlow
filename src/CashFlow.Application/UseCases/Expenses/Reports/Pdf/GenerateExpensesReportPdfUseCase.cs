...
public class GenerateExpensesReportPdfUseCase : IGenerateExpensesReportPdfUseCase
{
    ...
    public async Task<byte[]> Execute(DateOnly date)
    {
        ...
        var expenses = await this._repository.FilterByMonth(startDate, endDate);
        if (expenses.Count == 0)
        {
            return [];
        }
        ...
        foreach (var expense in expenses)
        {
            ...
            row.Cells[2].AddParagraph(expense.PaymentType.Translate());
    ...
    private Document CreateDocument(DateOnly date)
    {
        ...
        document.Info.Title = $"{ResourceReportGenerationMessages.EXPENSES_FOR} {date:Y}";
        document.Info.Author = "com.pedroaba.cashflow";
    ...
    private void CreateHeaderWithProfilePhotoAndName(Section section)
    {
        ...
        row.Cells[0].AddImage(
            Path.Combine(directoryName!, "Assets", "Photos", "profile.png")
        );

        row.Cells[1].AddParagraph("Hey, Pedro Augusto");
        row.Cells[1].Format.Font = new Font { Name = FontHelper.RALEWAY_BLACK, Size = 16 };
        row.Cells[1].VerticalAlignment = MigraDoc.DocumentObjectModel.Tables.VerticalAlignment.Center;
        row.Cells[1].Format.LeftIndent = "20";
    }

    private void CreateTotalSpentSection(Section section, DateOnly date, decimal totalExpenses)
    {
